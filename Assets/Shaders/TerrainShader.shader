Shader "Custom/Terrain" {
	...
	Properties{
		...
		_MainCol("Top Color", Color) = (1,1,1,1)
		_WallCol("Wall Color", Color) = (1,1,1,1)
	...
	SubShader{
		...
		void surf(Input IN, inout SurfaceOutputStandard o) {
			...
			float3 xP = tex2D(_MainTex, scaledWorldPos.yz) * _WallCol * pWeight.x;
			float3 yP = tex2D(_MainTex, scaledWorldPos.xz) * _MainCol * pWeight.y;
			float3 zP = tex2D(_MainTex, scaledWorldPos.xy) * _WallCol * pWeight.z;
